...
define('ORCID_AFFILIATION_URL', 'employments');
...
class OrcidProfilePlugin extends GenericPlugin {
    ...
    /**
     * Return the employment affiliations
     * @param $orcidiD string
     * @return $array
     */    
    function getOrcidProfileAffiliation($orcidiD) {
        $journal = Request::getJournal();
        
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_URL =>  $url = $this->getSetting($journal->getId(), 'orcidProfileAPIPath') . ORCID_API_VERSION_URL . urlencode($orcidiD) . '/' . ORCID_AFFILIATION_URL,
            CURLOPT_POST => false,
            CURLOPT_HTTPHEADER => array('Accept: application/json'),
        ));
        $result = curl_exec($curl);
    ...
    */
    ...
    function searchProfile($searchName, $searchLastname, $searchEmail, $page, $itemsPerPage) {
        ...
        if ($searchName || $searchLastname || $searchEmail) {
            ...
            if ($accessToken) {
                $searchParams = array('given-names' => $searchName,
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_POST => false,
                    CURLOPT_URL => $url = $this->getSetting($journal->getId(), 'orcidProfileAPIPath') . ORCID_API_VERSION_URL . 'search/' . '?' . $query,
                ));   
                $result = curl_exec($curl);
                if ($result) {
                    $returner = json_decode($result, true);
